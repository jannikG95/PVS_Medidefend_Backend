...
# JSON-Datei laden
with open("goz_data.json", "r", encoding="utf-8") as f:
    data = json.load(f)
...
vectors = []
for goz_index, (goz_number, goz_text) in enumerate(data.items(), start=1):
    # Generate the embedding for the text
    embedding = EMBEDDINGS_GPT.embed_query(goz_text)
    # Normalize the ID
    vec_id = normalize_id(goz_number, goz_index)
    # Append the result to the vectors list
    vectors.append({
        "id": vec_id,
        "values": embedding,
        "metadata": {"goz_number": goz_number, "goz_text": goz_text}
    })
